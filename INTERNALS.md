...
1. Bump `CefRedistVersion` / `CefRedistOSXVersion` / `CefRedistLinuxVersion` and `Version` in `Directory.Build.props` (version scheme: `<chromium major>.<CEF build>.<fork revision>`).
...
- `CefGlue.Common\CommonBrowserBehaviors.cs` is `Compile Remove`'d and included nowhere — dead source. Only `BaseCefBrowser.cs` is the shared-source file.
...
- `TestBase.cs`: `[OneTimeSetUp]` initializes CEF once per process (static guard) with a custom `test` scheme, then starts one background thread running Avalonia's `Dispatcher.UIThread.MainLoop` forever (`SetupWithoutStarting`). Each `[SetUp]` creates a real, shown 1×1 Avalonia `Window` hosting a real `AvaloniaCefBrowser` and awaits `BrowserInitialized`.
...
- `CefGlue.Demo.WPF` is much thinner: `[STAThread] Program.Main` must call `CefRuntimeLoader.Initialize` **before** the WPF `App` starts; the JS-interop menu handlers are commented out. Its custom `test:` scheme handler in the Avalonia demo throws `NotImplementedException` by design.
